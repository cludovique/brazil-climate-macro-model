...
<div class="tab-panel active" id="tab0">
  <div class="grid-4">
    ...
    <div class="card kpi"><div class="val" id="kpi-nsect">—</div><div class="lbl">Setores Activos</div><div class="sub">de 73</div></div>
  </div>
  <div class="grid-2">
    <div class="card">
...
// position:"top" keeps the legend above the bars; white text for readability.
...
function initOverview(){
  ...
  document.getElementById("kpi-nsect").textContent="73 / 73";
...
function initMultipliers(){
  ...
  const mpGrps=[...new Set(mp.map(r=>r[grpCol]).filter(Boolean))];
  const mpGrpC=mpGrps.map(gc);
  mkHBar("ch-mp-all",mp.map(r=>r.cod),
    [{label:"MP",data:mp.map(r=>r.MP),
      backgroundColor:mp.map(r=>setE.includes(r.cod)?"#F39C12":gc(r[grpCol])),borderRadius:2}],
    {plugins:{...CD.plugins,
...
function initExtraction(){
  ...
  // Build legend: unique groups in current scheme + energy highlight
  function extLegend(rows){
    const grps=[...new Set(rows.map(r=>r[grpCol]).filter(Boolean))];
    return makeColorLegend([...grps,"⚡ Energia"],[...grps.map(gc),"#F39C12"],"top");
  }
  function barColor(r){return setE.includes(r.cod)?"#F39C12":gc(r[grpCol]);}
