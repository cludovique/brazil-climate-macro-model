...
<div class="tab-panel" id="tab4">
  ...
  <div class="grid-2">
    ...
    <div class="card"><div class="card-title" id="ttl-alpha-energy">Intensidade Energ&eacute;tica (ktep/R$M)</div>
...
function showTab(n){
  ...
  const _tp=document.getElementById("tab"+n); if(_tp)_tp.classList.add("active");
...
function initEnergy(){
  ...
  const al=enrich(D.Energia_Alpha);
  setTtl("ttl-alpha-energy","Intensidade Energética "+(sec?"por Setor":"por "+s.label)+" (ktep/R$M)");
  if(sec){
    const alS=[...al].map(r=>({...r,tot:fontes.reduce((a,f)=>a+(r[f]||0),0)})).sort((a,b)=>b.tot-a.tot);
    mkHBar("ch-alpha-energy",alS.map(r=>r.cod),
      fontes.map(f=>({label:f,data:alS.map(r=>r[f]||0),backgroundColor:FONTE_C[f]||"#888",borderRadius:2,stack:"s"})),
      {scales:{x:{...CD.scales.x,stacked:true,title:{display:true,text:"ktep/R$M",color:"#8892a4"}},
               y:{...CD.scales.y,stacked:true}},
       plugins:{...CD.plugins,tooltip:TIP_SECTOR}});
  } else {
    const col=scCol(); const lbls=schLabels(al);
    const alG=lbls.map(lbl=>{
      const rows=al.filter(r=>r[col]===lbl); const n=rows.length||1;
      const obj={lbl,tot:0};
      fontes.forEach(f=>{obj[f]=rows.reduce((a,r)=>a+(r[f]||0),0)/n;});
      obj.tot=fontes.reduce((a,f)=>a+obj[f],0);
      return obj;
    }).sort((a,b)=>b.tot-a.tot);
    mkHBar("ch-alpha-energy",alG.map(r=>r.lbl),
      fontes.map(f=>({label:f,data:alG.map(r=>r[f]||0),backgroundColor:FONTE_C[f]||"#888",borderRadius:2,stack:"s"})),
      {scales:{x:{...CD.scales.x,stacked:true,title:{display:true,text:"ktep/R$M",color:"#8892a4"}},
               y:{...CD.scales.y,stacked:true}},
       plugins:{...CD.plugins,tooltip:TIP_SECTOR}});
  }
