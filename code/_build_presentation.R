# =============================================================================
# BUILD PPE PRESENTATION — MIP×MMA Model  (16:9 widescreen)
...
SW <- 12192000 / 914400   # 13.3333... inches (widescreen width)
SH <- 7.5                  # inches (height, same as 4:3)
...
rule  <- function(sl, col, l, t, w=2.67)
  ph_with(sl, fpar(), ph_location(left=l, top=t, width=w, height=0.06, bg=col))
# Thin vertical rule
vrule <- function(sl, col, l, t, h=2.0, w=0.07)
  ph_with(sl, fpar(), ph_location(left=l, top=t, width=w, height=h, bg=col))
# Full slide background
bg    <- function(sl, col=DARK) box(sl, col, 0, 0, SW, SH)
# Header bar + title
hdr   <- function(sl, txt, col=GREEN) {
  sl <- box(sl, col, 0, 0, SW, 0.9)
  tb(sl, txt, 0.5, 0.1, SW-1.0, 0.7, WHITE, 22, b=TRUE)
}
...
# ── Create presentation & set 16:9 slide size ──────────────────────────────────
prs <- read_pptx()
prs_xml <- prs$presentation$get()
sz <- xml_find_first(prs_xml, "//*[local-name()='sldSz']")
xml_set_attr(sz, "cx", "12192000")
xml_set_attr(sz, "type", "screen16x9")
...
for (i in 1:3) {
  xl <- 0.27 + (i-1)*4.36
  sl <- box(sl, DKBL, xl, 1.05, 4.20, 5.7)
  sl <- rule(sl, cols3[i], xl+0.16, 1.12, w=3.87)
  sl <- tb(sl, titles[i], xl+0.20, 1.22, 3.87, 0.55, WHITE, 12, b=TRUE)
  sl <- bl(sl, items3[[i]], xl+0.20, 1.82, 3.87, 4.7)
}
sl <- tb(sl, "Abordagem macroeconômica setorial, compatível com BLUES/EPE e metodologia IPCC AR5.",
             0.40, 6.9, 12.5, 0.4, MGRAY, 10, it=TRUE, align="center")
...
for (h_info in list(c("ENTRADAS",0.20,3.47), c("ENGINES",4.0,5.73), c("RESULTADOS",10.13,2.80))) {
  sl <- tb(sl, h_info[1], as.numeric(h_info[2]), 1.08, as.numeric(h_info[3]), 0.35,
               MGRAY, 10, b=TRUE, align="center")
}
# MMA box
sl <- box(sl, "#2A3F6A", 0.20, 1.5, 3.47, 2.2)
sl <- tb(sl, "Cenários MMA-SMC\nv3 AR5 (2024)", 0.20, 1.6, 3.47, 0.65, GREEN, 13, b=TRUE, align="center")
sl <- bl(sl, c("Mix energético 2025–2050","Volumes físicos (TWh, Mt)","Investimentos (bi USD)","GHG por setor"),
             0.27, 2.3, 3.33, 1.3, col=LGRAY, sz=10, bullet="• ")
# MIP box
sl <- box(sl, "#2A3F6A", 0.20, 3.9, 3.47, 2.2)
sl <- tb(sl, "MIP-EPE 2018\n73 Setores", 0.20, 4.0, 3.47, 0.65, AMBER, 13, b=TRUE, align="center")
sl <- bl(sl, c("Matriz A (coef. técnicos)","Leontief L = (I-A)⁻¹","Satélites: VA, Empregos","ALPHA: ktep/R$M (6 fontes)"),
             0.27, 4.65, 3.33, 1.35, col=LGRAY, sz=10, bullet="• ")
# Arrows
sl <- tb(sl, "→", 3.71, 2.35, 0.40, 0.45, MGRAY, 22, b=TRUE, align="center")
sl <- tb(sl, "→", 3.71, 3.9,  0.40, 0.45, MGRAY, 22, b=TRUE, align="center")
sl <- tb(sl, "→", 9.73, 3.3,  0.40, 0.5,  MGRAY, 22, b=TRUE, align="center")
# Engine boxes
for (en in list(
  ...
  sl <- box(sl, en[[2]], 4.0, en[[1]], 5.73, 1.35)
  sl <- tb(sl, en[[3]], 4.20, en[[1]]+0.08, 5.40, 0.5, WHITE, 12, b=TRUE)
  sl <- bl(sl, en[[4]], 4.20, en[[1]]+0.6, 5.40, 0.65, col=LGRAY, sz=10)
}
# Results box
sl <- box(sl, "#1D5038", 10.13, 1.5, 2.93, 5.1)
sl <- tb(sl, "RESULTADOS", 10.13, 1.6, 2.93, 0.4, GREEN, 11, b=TRUE, align="center")
sl <- bl(sl, c("Δ Produção (R$ tri)","Δ PIB / VA (R$ bi)","Δ Empregos (mi)",
             ...
             10.20, 2.1, 2.80, 4.2, col=LGRAY, sz=10, bullet="• ")
...
for (i in 1:4) {
  ...
  xl <- 0.40 + ci*3.27; yt <- 1.1 + (row-1)*1.9
  sl <- box(sl, DKBL, xl, yt, 3.07, 1.65)
  sl <- vrule(sl, k[[4]], xl+0.09, yt+0.15, h=1.3)
  sl <- tb(sl, k[[1]], xl+0.33, yt+0.15, 2.67, 0.55, k[[4]], 20, b=TRUE)
  sl <- tb(sl, k[[2]], xl+0.33, yt+0.72, 2.67, 0.38, WHITE,  11, b=TRUE)
  sl <- tb(sl, k[[3]], xl+0.33, yt+1.1,  2.67, 0.38, MGRAY,  9, it=TRUE)
}
# ALPHA panel — occupies right side
sl <- box(sl, DKBL, 7.0, 1.1, 6.07, 5.9)
sl <- tb(sl, "Satélite Energético — Matriz ALPHA", 7.27, 1.2, 5.60, 0.45, GREEN, 13, b=TRUE)
sl <- tb(sl, "ktep por R$ Milhão de produção  ×  73 setores  ×  6 fontes",
             7.27, 1.65, 5.60, 0.35, LGRAY, 10, it=TRUE)
for (i_f in 1:6) {
  ...
  yt <- 2.1 + (i_f-1)*0.77
  sl <- vrule(sl, fo[[2]], 7.30, yt, h=0.5)
  sl <- tb(sl, fo[[1]], 7.53, yt,      3.47, 0.3,  WHITE, 11, b=TRUE)
  sl <- tb(sl, fo[[3]], 7.53, yt+0.3,  3.47, 0.28, MGRAY, 9, it=TRUE)
}
sl <- tb(sl, "Uso: ALPHA × Δx = variação no consumo de energia por fonte (ktep)",
             7.27, 6.7, 5.60, 0.3, LGREE, 10, it=TRUE)
...
sl <- box(sl, DKBL, 0.27,  1.1, 6.20, 5.8)
sl <- tb(sl, "1a  •  Escala do PIB", 0.53, 1.2, 5.73, 0.5, AMBER, 14, b=TRUE)
...
sl <- box(sl, DKBL, 6.73, 1.1, 6.33, 5.8)
sl <- tb(sl, "1b  •  Investimento de Transição (FBCF)", 7.00, 1.2, 5.87, 0.5, AMBER, 14, b=TRUE)
...
sl <- tb(sl, "sc_combustível = share_cenário(t) / share_base(2020)",
             0.67, 1.0, 12, 0.45, "#378ADD", 14, b=TRUE, align="center")
sl <- tb(sl, "A*(j,t) = A(j) × sc    →    L*(t) = (I − A*)⁻¹    →    Δx = L* × Δf",
             0.67, 1.45, 12, 0.4, LGRAY, 12, it=TRUE, align="center")
...
for (i in 1:8) {
  ...
  xl <- 0.20 + ci*3.30; yt <- 2.05 + (ri-1)*2.5
  sl <- box(sl, DKBL, xl, yt, 3.13, 2.25)
  sl <- rule(sl, g[[2]], xl+0.13, yt+0.07, w=2.87)
  sl <- tb(sl, g[[1]], xl+0.16, yt+0.18, 2.87, 0.5,  WHITE, 11, b=TRUE)
  sl <- tb(sl, g[[3]], xl+0.16, yt+0.72, 2.87, 0.65, LGRAY, 9.5)
  sl <- tb(sl, g[[4]], xl+0.16, yt+1.45, 2.87, 0.55, MGRAY, 9, it=TRUE)
}
sl <- tb(sl, "normalize=TRUE preserva intensidade energética total (transporte, indústria)  |  normalize=FALSE permite redução genuína (geração elétrica −95% fóssil)",
             0.27, 7.1, 12.80, 0.35, MGRAY, 9, it=TRUE, align="center")
...
for (i in 1:6) {
  ...
  xl <- 0.27 + ci*4.36; yt <- 1.65 + (ri-1)*2.6
  sl <- box(sl, DKBL, xl, yt, 4.13, 2.4)
  sl <- tb(sl, it[[1]], xl+0.20, yt+0.12, 3.87, 0.55, AMBER, 11, b=TRUE)
  sl <- tb(sl, it[[2]], xl+0.20, yt+0.72, 3.87, 0.65, LGRAY, 10)
  sl <- tb(sl, it[[3]], xl+0.20, yt+1.42, 3.87, 0.70, MGRAY, 9.5, it=TRUE)
}
...
sl <- tb(sl, "Prêmio = efeito líquido 100D vs. crescimento de base (PIB sem transição — apenas escala GDP)",
             0.40, 0.95, 12.53, 0.4, LGRAY, 11, it=TRUE)
...
for (i in 1:5) {
  k <- res_kpis[[i]]; xl <- 0.20 + (i-1)*2.60
  sl <- box(sl, DKBL, xl, 1.45, 2.47, 1.6)
  sl <- tb(sl, k[[1]], xl+0.07, 1.52, 2.33, 0.55, k[[4]], 15, b=TRUE, align="center")
  sl <- tb(sl, k[[2]], xl+0.07, 2.09, 2.33, 0.35, WHITE,   9, b=TRUE, align="center")
  sl <- tb(sl, k[[3]], xl+0.07, 2.44, 2.33, 0.45, MGRAY,   8, it=TRUE, align="center")
}

sl <- tb(sl, "Trajetória por Cenário — Variação de Produção (ΔR$ bi vs. 2018)",
             0.40, 3.25, 12.53, 0.42, WHITE, 12, b=TRUE)
anos_sel <- c(2025,2030,2035,2040,2045,2050)
cens_sel <- c("0D","25D","100D")
col_w <- c(1.33, rep(1.87, 6)); col_x <- cumsum(c(0.40, col_w[-7]))
for (j in 1:7) {
  sl <- box(sl, "#0D1F3C", col_x[j], 3.7, col_w[j], 0.38)
  sl <- tb(sl, c("Cenário", as.character(anos_sel))[j],
               col_x[j]+0.04, 3.73, col_w[j]-0.08, 0.32, GREEN, 10, b=TRUE, align="center")
}
...
for (ri in seq_along(cens_sel)) {
  ...
  for (j in 1:7) {
    ...
    sl <- tb(sl, c(cn, vals)[j], col_x[j]+0.04, yt+0.1, col_w[j]-0.08, 0.38,
...
for (i in 1:5) {
  tb2 <- tabs[[i]]; xl <- 0.27 + (i-1)*2.60
  sl <- box(sl, DKBL, xl, 1.65, 2.47, 5.45)
  sl <- rule(sl, tb2[[3]], xl+0.13, 1.7, w=2.20)
  sl <- tb(sl, tb2[[1]], xl+0.07, 1.78, 2.33, 0.32, MGRAY, 9, b=TRUE, align="center")
  sl <- tb(sl, tb2[[2]], xl+0.07, 2.12, 2.33, 0.5, WHITE, 11, b=TRUE, align="center")
  sl <- bl(sl, tb2[[4]], xl+0.16, 2.7, 2.20, 4.2, col=LGRAY, sz=9, bullet="• ")
}
...
for (panel in list(list(lacunas, 0.27, "Lacunas do Modelo", AMBER),
                   list(colabs,  6.80, "Como Colaborar",   GREEN))) {
  sl <- box(sl, DKBL, panel[[2]], 1.0, 6.27, 6.25)
  sl <- tb(sl, panel[[3]], panel[[2]]+0.27, 1.08, 5.73, 0.45, panel[[4]], 14, b=TRUE)
  for (j in seq_along(panel[[1]])) {
    it <- panel[[1]][[j]]; yt <- 1.65 + (j-1)*1.1
    sl <- tb(sl, it[[1]], panel[[2]]+0.33, yt,      5.60, 0.35, WHITE, 10, b=TRUE)
    sl <- tb(sl, it[[2]], panel[[2]]+0.33, yt+0.33, 5.60, 0.7,  LGRAY, 9)
  }
}
...
sl <- bg(sl)
sl <- vrule(sl, GREEN, 0.47, 2.0, h=3.5)
sl <- tb(sl, "Obrigada!", 0.93, 2.1, 12, 1.0, GREEN, 40, b=TRUE)
sl <- tb(sl, "Código aberto — comentários, críticas e contribuições são bem-vindos.",
             0.93, 3.2, 11.5, 0.6, WHITE, 15)
lks <- list(
...
for (i in seq_along(lks)) {
  yt <- 3.95 + (i-1)*0.7
  sl <- tb(sl, lks[[i]][[1]], 0.93, yt, 3.87, 0.45, MGRAY, 12)
  sl <- tb(sl, lks[[i]][[2]], 5.07, yt, 7.87, 0.45, LGREE, 12)
}
...
cat(sprintf("Total slides: %d  |  Slide size: %.2f x %.2f in\n",
            length(prs), slide_size(prs)$width, slide_size(prs)$height))
